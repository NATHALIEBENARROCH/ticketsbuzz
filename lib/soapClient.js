...
// ──────────────────────────────────────────────────────────────────────────
// GetHighSalesPerformers  – returns top-selling performers for a category.
// Used by /api/featured to power the "no local events" fallback.
// ──────────────────────────────────────────────────────────────────────────
export async function getHighSalesPerformers(options = {}) {
  const params = {
    websiteConfigID: options.websiteConfigID ?? WCID,
    numReturned: options.numReturned ?? 6,
    parentCategoryID: options.parentCategoryID,
    childCategoryID: options.childCategoryID,
    grandchildCategoryID: options.grandchildCategoryID,
  };

  const soap = buildSoapEnvelope('GetHighSalesPerformers', params);
  ...
  async function requestWithTokenRetry(retryOn401 = true) {
    ...
    const response = await fetchWithTimeout(SOAP_ENDPOINT, {
      ...
      headers: {
        'Accept': 'text/xml',
        'Content-Type': 'text/xml; charset=utf-8',
        'SOAPAction': '"http://tnwebservices.ticketnetwork.com/tnwebservice/v4.0/GetHighSalesPerformers"',
        'Authorization': `Bearer ${token}`,
      },
      body: soap,
    });
  ...
  try {
    ...
    if (text.trimStart().startsWith('<!DOCTYPE html') || text.trimStart().startsWith('<html')) {
      throw new Error('SOAP endpoint returned HTML. Check credentials.');
    }
    ...
    let parseError = null;
    try {
      parsed = parseSoapResponse(text, 'GetHighSalesPerformers');
    } catch (error) {
      parseError = error.message;
    }

    return { status: response.status, xml: text, parsed, parseError };
  } catch (error) {
    console.error('❌ SOAP GetHighSalesPerformers error:', error.message);
    throw error;
  }
}
